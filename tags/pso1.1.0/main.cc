...
void simulatedAnnealing(Input &input, vector<MoleculeSet*> &moleculeSets, vector<MoleculeSet*> &bestNMoleculeSets, string &seedFiles)
{
	...
	const int numIterationsToSetTemp = 200;
	const int setTemperatureEvery = 20;
		...
		if (!fout.is_open())
		{
			...
			if (!bErrorToStopFor) {
				...
				if (input.m_bTestMode) {
					sprintf(testFileName, "structures/structure1_It%d.com", input.m_iIteration);
					tempString = testFileName;
					Energy::createGaussianInputFile(tempString, emptyString, emptyString, input, *moleculeSets[0], true);
				}
			}
...
/*			for (i = 0; i < (signed int)moleculeSets.size(); ++i)
		...
		{ 
			...
			fout << endl << "Best " << bestNMoleculeSets.size() << " structures:" << endl;
			...
			for (i = 0; i < (signed int)bestNMoleculeSets.size(); ++i)
			{
				...
				fout << setiosflags(ios::fixed) << setprecision(8) << "Energy: " << bestNMoleculeSets[i]->getEnergy() << endl << endl;
			}
...
int main(int argc, char *argv[])
{
	...
	if (strncmp(argv[1],"-t", 3) == 0) { // # Make an output file from a resume file that's readable
		...
		bestNMoleculeSets.clear();
		bestIndividualMoleculeSets.clear();
	} else if (strncmp(argv[1],"-c", 3) == 0) { // # Create .com files from an optimization or resume file

		if (argc != 5) {
			cout << "When using the -c option, please specify a resume or optimization file, followed by a directory where .com files will be written, followed by a prefix for the .com files." << endl;
			return 0;
		}
		...
		if (bestNMoleculeSets.size() > 100) {
			cout << "There are " << bestNMoleculeSets.size() << " structures in this file." << endl;
			cout << "Enter the number of structures you wish to create .com files for or type 'all': ";
			cin >> answer;
			if (strncmp(answer.c_str(),"all",3) == 0)
				numFiles = bestNMoleculeSets.size();
			else
		...
		for (i = 0; i < numFiles; ++i) {
			...
			Energy::createGaussianInputFile(tempOutputFileName, emptyString, sId, input, *bestNMoleculeSets[i], true);
		}
		cout << "Done!" << endl;
   		// Clean up
		for (i = 0; i < (signed int)moleculeSets.size(); ++i)
		...
		if (argc != 5) {
			cout << "When using the -o option, please specify a resume file, followed by the number of structures to optimize at a time, followed by the optimization file to be created." << endl;
			return 0;
		}
		inputFileName = argv[2];
		...
		sscanf(argv[3], "%d", &input.m_iStructuresToOptimizeAtATime);
		...
		if (strncmp(argv[1],"-s", 3) == 0) {
			...
			if ((strncmp(argv[1],"-test", 6) == 0) && (argc == 3)) {
				...
				sprintf(commandLine,"rm -r structures");
				system(commandLine);
				sprintf(commandLine,"mkdir structures");
				system(commandLine);
			} else if (argc > 2) {
