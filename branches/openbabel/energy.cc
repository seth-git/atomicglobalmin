...
OpenBabel::OBConversion Energy::s_obConversion;
...
//			s_scratchCommand = "env SCRDIR=" + s_fullScratchDirectory + " ";
		...
		break;
	case OPEN_BABEL:
		if (s_energyProgram.m_iNumOutputFileTypes == 0) {
			cout << "Please specify an output file extension in energyProgram.cc." << endl;
			return false;
		}
		if (!s_obConversion.SetInAndOutFormats(s_energyProgram.m_sOutputFileTypeExtensions[0].c_str(), s_energyProgram.m_sInputFileExtension.c_str()))
		{
			cout << "Input format: '" <<  s_energyProgram.m_sOutputFileTypeExtensions[0] << "' or output format: '" << s_energyProgram.m_sInputFileExtension
				 << "' is not supported by openbabel." << endl;
			return false;
		}
		break;
	}
...
//		break; -> Note that this break is commented out
	default:
		i = s_energyProgram.getNumParameters();
...
bool Energy::createInputFile(MoleculeSet &moleculeSet, int populationMemberNumber, bool resetInputFileName, bool writeMetaData)
{
	...
	case GAUSSIAN:
		...
		break;
	case OPEN_BABEL:
		createOpenBabelInputFile(moleculeSet, populationMemberNumber, writeMetaData);
		break;
	default:
...
bool Energy::createOpenBabelInputFile(MoleculeSet &moleculeSet, int populationMemberNumber, bool writeEnergyValueInHeader)
{
	OpenBabel::OBMol obMol;
	bool returnValue;
	
	moleculeSet.writeToOBMol(obMol);
	if (writeEnergyValueInHeader) {
		char title[100];
		snprintf(title, sizeof(title), "This is a computer generated structure with energy: %lf", moleculeSet.getEnergy());
		obMol.SetTitle(title);
	} else {
		obMol.SetTitle("This is a computer generated structure.");
	}
	obMol.SetTotalCharge(s_iCharge);
	obMol.SetTotalSpinMultiplicity((unsigned int)s_iMultiplicity);

	returnValue = s_obConversion.WriteFile(&obMol, moleculeSet.getInputEnergyFile());
	s_obConversion.CloseOutFile();
	
	return returnValue;
}

int Energy::readOutputFile(const char* outputFile, FLOAT &energy, MoleculeSet* pMoleculeSet) {
	switch (s_energyProgram.m_iProgramID) {
	case GAUSSIAN:
		return readGaussianOutputFile(outputFile, energy, pMoleculeSet);
	case OPEN_BABEL:
		return readOpenBabelOutputFile(outputFile, energy, pMoleculeSet);
	default:
...
int Energy::readOpenBabelOutputFile(const char* outputFile, FLOAT &energy, MoleculeSet* pMoleculeSet) {
	OpenBabel::OBMol* pOBMol;
	int returnValue = 0;
	
	if (!s_obConversion.ReadFile(pOBMol, outputFile) || (pOBMol == NULL))
		return returnValue;
	
	energy = pOBMol->GetEnergy();
