...
void Molecule::writeToOBMol(OpenBabel::OBMol &obMol)
{
	OpenBabel::OBAtom newAtom;
	for (int atomIndex = 0; atomIndex < m_iNumberOfAtoms; ++atomIndex) {
		newAtom.SetAtomicNum(m_atoms[atomIndex].m_iAtomicNumber);
		newAtom.SetVector(m_atoms[atomIndex].m_globalPoint.x, m_atoms[atomIndex].m_globalPoint.y, m_atoms[atomIndex].m_globalPoint.z);
		if (!obMol.AddAtom(newAtom))
		    break;
	}
}
