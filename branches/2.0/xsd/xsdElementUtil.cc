	...
	switch(m_type) {
		case XSD_ALL:
			...
			for (pElem = handle.FirstChild().Element(); pElem; pElem=pElem->NextSiblingElement())
			{
				...
				for (i = 0; i < m_iElements; ++i) {
					if (strcmp(m_elementNames[i], pName) == 0) {
						if (m_allElements[i] != NULL) {
							printf(messagesDL->m_sMaxOf1ElementExceeded.c_str(), m_elementNames[i], m_sParentElement, pElem->Row());
							return false;
						}
				...
				if (!bMatch) {
					printf(messagesDL->m_sUnrecognizedElement.c_str(), pName, m_sParentElement, pElem->Row());
					return false;
				}
			}
			...
			for (i = 0; i < m_iElements; ++i) {
				if (m_allElements[i] == NULL && m_minOccurs[i] == 1) {
					printf(messagesDL->m_sElementNumNot1.c_str(), m_elementNames[i], m_sParentElement, handle.Element()->Row());
					return false;
				}
			}
		...
		case XSD_CHOICE:
			...
			if (pElem) {
				printf(messagesDL->m_sElementRequiresExactly1Child.c_str(), m_sParentElement, pElem->Row());
				return false;
			}
			break;
		case XSD_SEQUENCE:
			...
			while (pElem) {
				...
				while (strcmp(m_elementNames[i], pName) != 0) {
					if (m_sequenceElements[i].size() < m_minOccurs[i]) {
						if (m_minOccurs[i] > 1) {
							printf(messagesDL->m_sElementRequiresNChildren.c_str(), m_minOccurs[i], m_elementNames[i], m_sParentElement, pElem->Row());
						} else if (m_minOccurs[i] > 0) {
							printf(messagesDL->m_sElementRequires1ChildMin.c_str(), m_elementNames[i], m_sParentElement, pElem->Row());
						}
						printSequenceError();
						return false;
					}
					++i;
					if (i >= m_iElements) {
						printf(messagesDL->m_sMisplacedElement.c_str(), pName, m_sParentElement, pElem->Row());
						printSequenceError();
						return false;
					}
				}
				...
				if (m_maxOccurs[i] != XSD_UNLIMITED && m_sequenceElements[i].size() == m_maxOccurs[i]) {
					printf(messagesDL->m_sMaxElementsExceeded.c_str(), m_maxOccurs[i], pName, m_sParentElement, pElem->Row());
					return false;
				}
...
void XsdElementUtil::printChoiceError(int lineNumber) {
	...
	printf(messagesDL->m_sChoiceError.c_str(), lineNumber, m_sParentElement, choiceElements.c_str());
}

void XsdElementUtil::printSequenceError() {
	const Strings* messagesDL = Strings::instance();
	if (m_iElements > 1) {
		printf(messagesDL->m_sChoiceElementOrder.c_str(), m_sParentElement);
