env = Environment(CCFLAGS='-Wall -Wno-unknown-pragmas -Wno-format -O3',LIBS='-lrt')
...
src = [Glob('*.cc'), Glob('translation/*.cc'), Glob('xsd/*.cc'), Glob('handbook/*.cc'), Glob('matrix/*.cc'), Glob('externalEnergy/*.cc')] + tinyxml + random
