...
const char GamessUS::s_sPathToExecutable[] = "/Full/path/to/Gamess/executable";
const char GamessUS::s_sInputFileExtension[] = "gamin";
const char GamessUS::s_sOutputFileExtension[] = "gamout";

const std::string GamessUS::s_sAtomsReplacement = "!atoms";

GamessUS::GamessUS(const ExternalEnergyXml* pExternalEnergyXml) : ExternalEnergy(pExternalEnergyXml) {
}

GamessUS::~GamessUS() {
}

bool GamessUS::createInputFile(const char* inputFileName, const Structure &structure) {
	std::string atoms;
	unsigned int i;
	...
	for (i = 0; i < n; ++i) {
		...
		snprintf(buffer, sizeof(buffer), "%-2s %2u %0.6lf %0.6lf %0.6lf\n", Handbook::getAtomicSymbol(atomicNumbers[i]), atomicNumbers[i], coordinate[0], coordinate[1], coordinate[2]);
		atoms.append(buffer);
	}
...
bool GamessUS::execute(Structure &structure) {
	using namespace std;
